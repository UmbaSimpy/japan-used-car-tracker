"""
CarSensor Toyota Voxy (current-gen, 90-series) Hybrid S-Z 2WD price scraper.
...
each vehicle with Voxy-specific weights/anchors, and writes a snapshot
to data/voxy_data.js.
...
Usage:
    python voxy_scraper.py              # scrape all pages (default)
    python voxy_scraper.py --pages 10   # limit pages (faster, for testing)
"""
...
BASE_URL             = "https://www.carsensor.net/usedcar/bTO/s021/spH/"
DATA_FILE            = Path(__file__).parent / "data" / "voxy_data.js"
...
TARGET_GRADES = {
    "1.8 ハイブリッド S-Z E-Four": "_skip_4wd",
...
# The multi-view camera is no longer a weighted factor — it's now an accessory
# (see OPTION_PACKAGES, "PVM"); its old 0.08 weight was folded into price.
WEIGHTS = {
...
# ── Desirable option packages / accessories (Voxy — same as Noah) ─────────────
# Each adds a CUMULATIVE value bonus — a car with several stacks all of them.
# Detected from the listing text and surfaced as purple badges. (label, bonus, regex)
# "PVM" (Panoramic View Monitor / multi-view camera) is worth less than the
# Advanced Park combo (which already includes it) and is de-duplicated against it
# in _extract_details so a car never gets both. (label, bonus, regex)
OPTION_PACKAGES = [
    ...
    ("PVM",            0.15, re.compile(r"マルチビューカメラ|マルチビュー|アラウンドビュー|全周囲カメラ|全方位カメラ|パノラミックビュー")),
]
...
def _extract_details(item) -> dict:
    ...
    # Desirable option packages / accessories (cumulative) — match each.
    # The multi-view camera (PVM) is one of these accessories now, not a weighted
    # factor. NOTE: the PVM regex deliberately avoids "360°" (CarSensor's photo
    # badge). De-dup: Advanced Park already bundles the PVM, so drop the standalone
    # PVM when Adv.パーク+PVM is present — a car never earns the PVM bonus twice.
    opts = [label for label, _, rx in OPTION_PACKAGES if rx.search(full_text)]
    if "Adv.パーク+PVM" in opts and "PVM" in opts:
        opts.remove("PVM")
...
def score_vehicle(vehicle: dict, price_bounds: dict[str, tuple[float, float]]) -> tuple[float, dict]:
    ...
    # Cumulative accessory bonus (快適利便PKG / Advanced Drive / Advanced Park +
    # PVM / standalone PVM multi-view camera). Stacks; clamped to 10.
...
def _clean_vehicle(v: dict) -> dict:
    """Serialize a scored vehicle dict for storage in voxy_data.js."""
    return {
        "score":           v["score"],
...
def load_existing() -> dict:
    ...
    m = re.search(r"window\.VOXY_DATA\s*=\s*(\{.*\});", src, re.DOTALL)
    if not m:
        print("  [!] Could not parse existing voxy_data.js — starting fresh")
        return _default_structure()
...
def save(data: dict) -> None:
    ...
    js = (
        ...
        f"window.VOXY_DATA = {json.dumps(data, ensure_ascii=False, indent=2)};\n"
    )
...
def run(max_pages: int | None) -> None:
    ...
    raw_candidates: dict[str, list[dict]] = {
        ...
        # (Voxy is a 2022+ design — used units commonly have higher km than the
